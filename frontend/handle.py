        ...
        try:
            ...
            if isinstance(recMsg, receive.Msg):
                ...
                if recMsg.MsgType == 'text':
                    str = recMsg.Content
                    if len(str) == 11 and str[0:1] == '1' and str.isdigit():
                        content = "您的联系方式已记录，下面开始答题笔试。\n"
                        backend.apply_interview(str, str, toUser)
                        problem = backend.online_interview(-1, "", toUser)
                        content = content + problem['problem'].encode('utf-8') + "\n请回复【】内的编号完成答题。"
                        replyMsg = reply.TextMsg(toUser, fromUser, content)
                        return replyMsg.send()
                    if len(str) >= 2 and str[:-1].isdigit() and (str[-1:] == 'A' or str[-1:] == 'B' or str[-1:] == 'C' or str[-1:] == 'D'
                    ...
                    if str[0:1] == "1":
                        ...
                        content = "欢迎使用"+company+"公司智能招聘系统！\n您可以随意发问，文字语音都行。\n如有意向请输入“我要投递”，可以直接答题笔试哦！"
                        replyMsg = reply.TextMsg(toUser, fromUser, content)
                        return replyMsg.send()
                    if str[0:1] == "2":
